...
@Composable
fun EcgScanner(navController: NavController) {
    val filePickerLauncher =
        rememberLauncherForActivityResult(contract = ActivityResultContracts.GetContent()) { uri: Uri? ->
            println("Selected file: $uri")
        }

    Scaffold(topBar = { DefaultTopAppBar(title = "ECG Scanner", navController = navController) },
        bottomBar = {
            BottomAppBar(containerColor = Color.Transparent, tonalElevation = 8.dp) {
                Spacer(Modifier.weight(1f))
                Button(
                    onClick = { filePickerLauncher.launch("text/csv") }, // Only allow .csv files to be selected
                    modifier = Modifier
                        .fillMaxWidth()
